...
const createSpaceship = (name, pilot, crewLimit) => {
    const spaceship = {
        name,
        pilot,
        crewLimit,
...
const getSpaceshipInfoFromPrompt = () => {
...
const getNewSpaceshipMemberInfoFromPrompt = () => {
...
const findSpaceshipToAddMember = () => {
    ...
    if (spaceshipFounded) {
        const isCorrectSpaceship = confirm(`Spaceship found:
...
const findSpaceshipByName = (name) => {
    return allSpaceships.find((spaceship) => spaceship.name === name);
};
const sendSpaceshipToMission = () => {
